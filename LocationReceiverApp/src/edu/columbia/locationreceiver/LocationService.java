...
public class LocationService {
	...
	public void saveCompensatedReadings(CompReading reading){
	...
	public void saveLocationDetails(JSONObject locationObj) {
		...
		System.out.println("in saveLocationDetails");
		...
		if (deviceObj != null) {
			deviceInfo = new DeviceInfo(deviceObj.getString(Constants.OS_VERSION), 
		...
		if (locationObj.has(Constants.PRESSURE_LIST)) {
			...
			JSONArray pressureArray = locationObj.getJSONArray(Constants.PRESSURE_LIST);
			for(int i=0; i<pressureArray.length(); i++) {
				...
				if(pressureObj != null) {
					...
					pressure.setPressure(pressureObj.getDouble(Constants.PRESSURE));
			...
			/*for(int i=0; i<wifiArray.length(); i++) {
				...
				for(int j=0; j<wifiArray.length(); j++) {
					JSONObject wReading = wifiArray.getJSONObject(j);
			...
			JSONArray coordinatesArray = locationObj.getJSONArray(Constants.COORDINATES_LIST);
			for(int i=0;i<coordinatesArray.length();i++) {
				...
				if(coordinatesObj != null) {
					...
					coordinates.setLatitude(coordinatesObj.getDouble(Constants.LATITUDE));
					coordinates.setLongitude(coordinatesObj.getDouble(Constants.LONGITUDE));
					coordinates.setTimestamp(coordinatesObj.getLong(Constants.TS));
					coordinates.setAccuracy(new Float(coordinatesObj.getDouble(Constants.ACCURACY)));
					coordinates.setAltitude(coordinatesObj.getDouble(Constants.ALTITUDE));
					coordinates.setSpeed(new Float(coordinatesObj.getDouble(Constants.SPEED)));
					coordinates.setProvider(coordinatesObj.getString(Constants.PROVIDER));					
			...
			JSONArray magnetometerArray = locationObj.getJSONArray(Constants.MAGNETOMETER_LIST);
			for(int i=0;i<magnetometerArray.length();i++) {
				...
				if(magnetometerObj != null) {
					...
					magnetometerReading.setX(magnetometerObj.getDouble(Constants.MAGNETOMETER_X));
					magnetometerReading.setY(magnetometerObj.getDouble(Constants.MAGNETOMETER_Y));
					magnetometerReading.setZ(magnetometerObj.getDouble(Constants.MAGNETOMETER_Z));
					magnetometerReading.setTimestamp(magnetometerObj.getLong(Constants.TS));
			...
			JSONObject locationDetailsObj = locationObj.getJSONObject(Constants.LOCATIONINFO);
			locationDetails.setName(locationDetailsObj.getString(Constants.NAME));
			locationDetails.setBuilding(locationDetailsObj.getString(Constants.BUILDING));
			locationDetails.setFloor(locationDetailsObj.getString(Constants.FLOOR));
			locationDetails.setRoom(locationDetailsObj.getString(Constants.ROOM));
			locationDetails.setStreetAddress(locationDetailsObj.getString(Constants.STREET_ADDRESS));
			locationDetails.setCity(locationDetailsObj.getString(Constants.CITY));
			locationDetails.setZipCode(locationDetailsObj.getString(Constants.ZIPCODE));
			locationDetails.setTimestamp(locationDetailsObj.getLong(Constants.TIMESTAMP));
		...
		locationDetails.setFloor(locationDetailsObj.getString(Constants.FLOOR));
		locationDetails.setRoom(locationDetailsObj.getString(Constants.ROOM));
		info.setLocationDetails(locationDetails);
		...
		for(int i=0; i<pressureArray.length(); i++) {
			PressureReading pressure = null;
			//JSONObject pressureObj = locationObj.getJSONObject(Constants.PRESSURE_READING);
			JSONObject pressureObj = pressureArray.getJSONObject(i);
			if(pressureObj != null) {
				pressure = new PressureReading();
				pressure.setPressure(pressureObj.getDouble(Constants.PRESSURE));
		...
		info.setPressureReadingList(pressureList);
		...
		info.setMagnetometerReadingList(magnetometerList);
		...
		for(int i=0;i<coordinatesArray.length();i++) {
			LocationCoordinates coordinates = null;
			//JSONObject coordinatesObj = locationObj.getJSONObject(Constants.COORDINATES);
			JSONObject coordinatesObj = coordinatesArray.getJSONObject(i);
			if(coordinatesObj != null) {
				coordinates = new LocationCoordinates();
				coordinates.setLatitude(coordinatesObj.getDouble(Constants.LATITUDE));
		...
		info.setLocationCoordinatesList(coordinatesList);
		
		
		//set wifi details
		List<List<WifiReading>> wifiReadingsList = new ArrayList<List<WifiReading>>();
		JSONArray wifiArray = locationObj.getJSONArray(Constants.WIFI_LIST);
		for(int i=0; i<wifiArray.length(); i++) {
			List<WifiReading> wifiList = new ArrayList<WifiReading>(); 
			//JSONArray wifiReadings = locationObj.getJSONArray(Constants.WIFILIST);
			JSONArray wifiReadings = wifiArray.getJSONArray(i);
			for(int j=0; j<wifiReadings.length(); j++) {
				JSONObject wReading = wifiReadings.getJSONObject(j);
				WifiReading reading = new WifiReading();
				reading.setSSID(wReading.getString(Constants.SSID));
				reading.setFrequency(wReading.getDouble(Constants.FREQUENCY));
				reading.setLevel(wReading.getInt(Constants.LEVEL));
			...
			//info.setWifiList(wifiList);
			wifiReadingsList.add(wifiList);
		}
		info.setWifiReadingsList(wifiReadingsList);
		
		locationDAO.saveLocation(info);*/
	}
}
